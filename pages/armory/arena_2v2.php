...
<head>
    ...
    <style>
        ...
        /* Search button - Red theme matching 2v2 nav button */
        #search-btn {
            cursor: pointer;
        ...
        <div class="glass-container">
            ...
            <form method="get" class="mb-8 flex flex-col sm:flex-row justify-center items-center gap-3">
                <input 
                    ...
                    placeholder="<?php echo translate('arena_2v2_search_placeholder', 'Search team name...'); ?>"
                    maxlength="16"
                ...
                <button 
                    type="submit"
                    id="search-btn"
                    class="px-6 py-2.5 rounded-lg transition-all duration-300 shadow-lg shadow-[rgba(239,68,68,0.3)] flex items-center gap-2"
                >
                    <i class="fas fa-search"></i> <?php echo translate('arena_2v2_search_btn', 'Search'); ?>
                </button>

                <?php if ($search !== ''): ?>
                    <a href="<?php echo $base_path; ?>armory/arena_2v2" class="reset-btn px-5 py-2.5 rounded-lg transition-all duration-300 flex items-center gap-2">
                        <i class="fas fa-times"></i> <?php echo translate('arena_2v2_reset_btn', 'Reset'); ?>
                    </a>
                <?php endif; ?>
            </form>
            ...
            <div class="table-container overflow-x-auto border border-[rgba(201,162,39,0.15)] shadow-2xl">
                <table class="w-full text-sm md:text-base text-center min-w-[700px]">
                    <thead class="bg-gradient-to-r from-[rgba(201,162,39,0.9)] to-[rgba(160,130,30,0.9)] text-amber-100 uppercase text-xs md:text-sm">
                        <tr>
                            <th class="py-4 px-3 md:px-6 font-bold whitespace-nowrap"><?php echo translate('arena_2v2_rank', 'Rank'); ?></th>
                            <th class="py-4 px-3 md:px-6 font-bold text-left whitespace-nowrap"><?php echo translate('arena_2v2_name', 'Name'); ?></th>
                            <th class="py-4 px-3 md:px-6 font-bold whitespace-nowrap"><?php echo translate('arena_2v2_faction', 'Faction'); ?></th>
                            <th class="py-4 px-3 md:px-6 font-bold whitespace-nowrap"><?php echo translate('arena_2v2_wins', 'Wins'); ?></th>
                            <th class="py-4 px-3 md:px-6 font-bold whitespace-nowrap"><?php echo translate('arena_2v2_losses', 'Losses'); ?></th>
                            <th class="py-4 px-3 md:px-6 font-bold whitespace-nowrap"><?php echo translate('arena_2v2_winrate', 'Winrate'); ?></th>
                            <th class="py-4 px-3 md:px-6 font-bold whitespace-nowrap"><?php echo translate('arena_2v2_rating', 'Rating'); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <?php else: ?>
                            ...
                            foreach ($teams as $team) {
                                ...
                                echo "<tr class='{$rowClass} transition-all duration-200 border-b border-gray-700/30 last:border-0' onclick=\"window.location='{$teamUrl}';\" style=\"cursor:pointer;\">
                                    ...
                                    <td class='py-3.5 px-3 md:px-6 text-[#2ecc71] font-semibold whitespace-nowrap'>{$team['seasonWins']}</td>
                                    <td class='py-3.5 px-3 md:px-6 text-[#ef4444] font-semibold whitespace-nowrap'>{$team['seasonLosses']}</td>
                                    <td class='py-3.5 px-3 md:px-6 font-bold text-[#f2cf5b] whitespace-nowrap'>{$team['winrate']}%</td>
                                    <td class='py-3.5 px-3 md:px-6 font-extrabold text-[#f2cf5b] text-base md:text-lg whitespace-nowrap'>{$team['rating']}</td>
                                </tr>";
                                $rank++;
                            }
                            ?>
            ...
            <div class="mt-6 text-center text-gray-400 text-xs md:text-sm">
                ...
                <?php echo translate('arena_2v2_footer', 'Click on any row to view team details.'); ?>
            </div>
        </div>
    </div>
</div>
