...
$page_class = 'shop'; // Set page class for CSS
...
// Define category images (excluding All)
$category_images = [
...
// Fetch shop items with specific columns, including at_login_flags
$query = "
...
// Check for success/error messages from purchase
$status_message = '';
if (isset($_GET['status'])) {
    switch ($_GET['status']) {
        case 'success':
            $status_message = '<p class="status success">Purchase successful!</p>';
            break;
        case 'insufficient_funds':
            $status_message = '<p class="status error">Insufficient points or tokens.</p>';
            break;
        case 'out_of_stock':
            $status_message = '<p class="status error">Item is out of stock.</p>';
            break;
        case 'error':
            $status_message = '<p class="status error">An error occurred during purchase. Check server logs for details.</p>';
            break;
        case 'Database query error':
            $status_message = '<p class="status error">Database error occurred. Contact support.</p>';
            break;
        case 'character_online':
            $status_message = '<p class="status error">Selected character must be logged out to complete the purchase.</p>';
            break;
        case 'level_too_high':
            $status_message = '<p class="status error">Your character\'s level is too high for this level boost.</p>';
            break;
        case 'character_not_found':
            $status_message = '<p class="status error">Selected character not found or not owned.</p>';
            break;
        case 'cooldown_active':
            $status_message = '<p class="status error">Please wait 5 seconds before making another purchase.</p>';
            break;
    }
}
            ...
            <a href="#" 
               ...
               data-category="<?php echo htmlspecialchars($category); ?>">
                <?php if (isset($category_images[$category])): ?>
                    <img src="<?php echo $base_path . $category_images[$category]; ?>" alt="<?php echo htmlspecialchars($category); ?> Icon" class="category-icon">
                <?php endif; ?>
                ...
                <div class="item-grid">
                    <?php foreach ($category_items as $item): ?>
                        <div class="item-card" data-entry="<?php echo $item['sit_entry'] ? htmlspecialchars($item['sit_entry']) : ''; ?>">
                            <img src="<?php echo $base_path . ($item['image'] ?? 'img/shop/placeholder.png'); ?>" alt="<?php echo htmlspecialchars($item['name']); ?>">
                            ...
                            <?php if ($category === 'Service'): ?>
                                <?php if ($item['level_boost'] !== null): ?>
                                    <p class="level-boost">Level Boost: <?php echo $item['level_boost']; ?></p>
                                <?php endif; ?>
                                <?php if ($item['at_login_flags'] & 1): ?>
                                    <p class="rename-character">Character Rename</p>
                                <?php endif; ?>
                                <?php if ($item['at_login_flags'] & 2): ?>
                                    <p class="reset-spells">Reset Spells</p>
                                <?php endif; ?>
                                <?php if ($item['at_login_flags'] & 4): ?>
                                    <p class="reset-talents">Reset Talents</p>
                                <?php endif; ?>
                                <?php if ($item['at_login_flags'] & 8): ?>
                                    <p class="customize-character">Character Customization</p>
                                <?php endif; ?>
                                <?php if ($item['at_login_flags'] & 16): ?>
                                    <p class="reset-pet-talents">Reset Pet Talents</p>
                                <?php endif; ?>
                                <?php if ($item['at_login_flags'] & 32): ?>
                                    <p class="first-login">First Login</p>
                                <?php endif; ?>
                                <?php if ($item['at_login_flags'] & 64): ?>
                                    <p class="faction-change">Faction Change</p>
                                <?php endif; ?>
                                <?php if ($item['at_login_flags'] & 128): ?>
                                    <p class="race-change">Race Change</p>
                                <?php endif; ?>
                            <?php endif; ?>
                            ...
                            <?php if (!empty($_SESSION['user_id'])): ?>
                                <form action="<?php echo $base_path; ?>pages/buy_item.php" method="POST" class="purchase-form">
                                    ...
                                    <?php else: ?>
                                        <p class="no-characters">No characters available.</p>
                                    <?php endif; ?>
                                    <button type="submit" class="buy-button" 
                                            <?php echo ($item['stock'] === 0 && $item['stock'] !== null) || empty($characters) || $cooldown_active ? 'disabled' : ''; ?>
                                            data-item-id="<?php echo $item['item_id']; ?>">
                                        <?php echo $cooldown_active ? 'Wait ' . $remaining_cooldown . 's' : 'Buy Now'; ?>
                                    </button>
                                </form>
                            <?php else: ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Purchase form validation and cooldown logic
    ...
    function updateBuyButtons(disabled, seconds) {
        document.querySelectorAll('.buy-button:not([href])').forEach(button => {
            if (!button.hasAttribute('disabled') || button.getAttribute('disabled') === '') {
                ...
                button.textContent = disabled ? `Wait ${seconds}s` : 'Buy Now';
    ...
    purchaseForms.forEach(form => {
        form.addEventListener('submit', function(e) {
            ...
            if (characterSelect && !characterSelect.value) {
                e.preventDefault();
                alert('Please select a character to purchase this item.');
                return;
            }
            ...
            if (isPurchaseBlocked) {
                e.preventDefault();
                alert('Please wait ' + remainingCooldown + ' seconds before making another purchase.');
                return;
            }
        });
    });
