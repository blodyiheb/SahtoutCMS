...
$stmt = $world_db->prepare("SELECT * FROM item_template WHERE entry = ?");
...
$socketColors = [
    1 => ['name' => 'Meta', 'icon' => '/Sahtout/img/shopimg/itemsicons/socketicons/socket_meta.gif'],
    2 => ['name' => 'Red', 'icon' => '/Sahtout/img/shopimg/itemsicons/socketicons/socket_red.gif'],
    4 => ['name' => 'Yellow', 'icon' => '/Sahtout/img/shopimg/itemsicons/socketicons/socket_yellow.gif'],
    8 => ['name' => 'Blue', 'icon' => '/Sahtout/img/shopimg/itemsicons/socketicons/socket_blue.gif']
];
...
<div style="background:#1a1a1a;border:1px solid #444;padding:8px;width:300px;color:#ccc;font:12px Arial;border-radius:4px;font-family:FrizQuadrata,Arial,sans-serif;">
    <div style="display:flex;justify-content:space-between;gap:8px;">
        ...
        <div>
            ...
            <?php if ($level): ?><div style="color:rgba(224, 184, 2, 0.993);">Item Level <?= $level ?></div><?php endif; ?>
        </div>
        <!-- Right Side -->
    ...
    <?php
    $resistances = ['Holy' => $item['holy_res'], 'Fire' => $item['fire_res'],
    'Nature' => $item['nature_res'], 'Frost' => $item['frost_res'],
    'Shadow' => $item['shadow_res'], 'Arcane' => $item['arcane_res']];
    ...
    <?php for ($i = 1; $i <= 3; $i++):
    $colorCode = $item["socketColor_$i"];
    if (isset($socketColors[$colorCode])):
        $colorName = $socketColors[$colorCode]['name'];
        $iconPath = $socketColors[$colorCode]['icon']; ?>
        <div style="color:#888;">
            <img src="<?= $iconPath ?>" style="width:16px;height:16px;max-width:16px;max-height:16px;object-fit:contain;vertical-align:middle;margin-right:6px;">
            Socket: <span style="color:<?= strtolower($colorName) ?>;"><?= $colorName ?></span>
        </div>
    <?php endif; endfor; ?>
    
    <?php if ($item['socketBonus']): ?>
        <div style="color:#888;">Socket Bonus: Spell ID <?= $item['socketBonus'] ?></div>
    <?php endif; ?>
    
    <?php if ($dur > 0): ?><div>Durability <?= $dur ?>/<?= $dur ?></div><?php endif; ?>
    <?php if ($requiredClassesText): ?><div style="color: rgba(235, 5, 5, 0.94);"><?= $requiredClassesText ?></div><?php endif; ?>
