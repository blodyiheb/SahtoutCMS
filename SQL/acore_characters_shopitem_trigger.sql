...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1
        FROM characters
        WHERE guid = p_character_guid
    ) THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Invalid character GUID';
    END IF;

    /*
     * Get item information from the world database.
     */
    SELECT
        CONCAT_WS(' ',
    ...
    /*
     * AzerothCore expects the charges string in the item_instance table.
     */
    IF v_charges IS NULL
       OR v_charges = ''
       OR v_charges = '0 0 0 0 0' THEN

        SELECT
            CASE
                WHEN spellcharges_1 = -1
                     AND spellid_1 > 0
                THEN CONCAT(spellid_1, ' 0 0 0 0')
                ELSE '0 0 0 0 0'
            END
        INTO v_charges
        FROM acore_world.item_template
    ...
    /*
     * IMPORTANT:
     * buy_item.php already started the transaction on $char_db.
     *
     * Therefore this procedure MUST NOT call START TRANSACTION
     * or COMMIT. The PHP transaction controls everything.
     */

    /*
     * Generate the next database GUID.
     *
     * This preserves your current schema where item_instance.guid
     * is NOT AUTO_INCREMENT.
     */
    SELECT COALESCE(MAX(guid), 0) + 1
    INTO v_item_guid
    FROM item_instance
    FOR UPDATE;
